(function () {
    ...
    function startUpdater() {
      ...
      if (Lampa.Platform.is("android")) {
        ...
        var checkUpdate = function checkUpdate() {
          ...
          network['native']("http://psmurashov.github.io/bata/api/info.json", function (found) {
            if (found) {
              if (semverCompare(found.version, current_version) > 0) {
                if (autoCheck) {
                  Lampa.Noty.show("Доступно обновление клиента Lampa TV [Premium]");
                } else {
    ...
    if (Lampa.Storage.get('jackett_url').toUpperCase() == 'JACRED.CF') Lampa.Storage.set('jackett_url', 'http://psmurashov.github.io/bata/app')
